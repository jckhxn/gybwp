...
var mapToEditLinks = require('./_chunks/mapToEditLinks-vYtU8SXd.cjs');
...
function createTranscoder(config) {
  ...
  const createEditLink = mapToEditLinks.defineEditLink(studioUrl);
  const report = {
    encoded: [],
    skipped: []
  };
  ...
  return (result, csm) => {
    ...
    return {
      result: mapToEditLinks.encode(result, csm, (value, sourceDocument, path) => transcode(value, sourceDocument, path)),
      report
    };
  };
}
...
exports.mapToEditLinks = mapToEditLinks.mapToEditLinks;
