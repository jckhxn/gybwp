...
var jsxRuntime = require("react/jsx-runtime"), noop = require("lodash/noop.js"), react = require("react"), slate = require("slate"), slateReact = require("slate-react"), debug$l = require("debug"), types = require("@sanity/types"), isEqual = require("lodash/isEqual.js"), content = require("@sanity/util/content"), styledComponents = require("styled-components"), uniq = require("lodash/uniq.js"), rxjs = require("rxjs"), schema = require("@sanity/schema"), get = require("lodash/get.js"), isUndefined = require("lodash/isUndefined.js"), omitBy = require("lodash/omitBy.js"), flatten$1 = require("lodash/flatten.js"), isObject = require("lodash/isObject.js"), isString = require("lodash/isString.js"), findIndex = require("lodash/findIndex.js"), clone = require("lodash/clone.js"), omit = require("lodash/omit.js"), isHotkeyEsm = require("is-hotkey-esm"), blockTools = require("@sanity/block-tools"), isPlainObject = require("lodash/isPlainObject.js"), throttle = require("lodash/throttle.js"), debounce = require("lodash/debounce.js");
function _interopDefaultCompat(e) {
  return e && typeof e == "object" && "default" in e ? e : { default: e };
}
var noop__default = /* @__PURE__ */ _interopDefaultCompat(noop), debug__default = /* @__PURE__ */ _interopDefaultCompat(debug$l), isEqual__default = /* @__PURE__ */ _interopDefaultCompat(isEqual), uniq__default = /* @__PURE__ */ _interopDefaultCompat(uniq), get__default = /* @__PURE__ */ _interopDefaultCompat(get), isUndefined__default = /* @__PURE__ */ _interopDefaultCompat(isUndefined), omitBy__default = /* @__PURE__ */ _interopDefaultCompat(omitBy), flatten__default = /* @__PURE__ */ _interopDefaultCompat(flatten$1), isObject__default = /* @__PURE__ */ _interopDefaultCompat(isObject), isString__default = /* @__PURE__ */ _interopDefaultCompat(isString), findIndex__default = /* @__PURE__ */ _interopDefaultCompat(findIndex), clone__default = /* @__PURE__ */ _interopDefaultCompat(clone), omit__default = /* @__PURE__ */ _interopDefaultCompat(omit), isPlainObject__default = /* @__PURE__ */ _interopDefaultCompat(isPlainObject), throttle__default = /* @__PURE__ */ _interopDefaultCompat(throttle), debounce__default = /* @__PURE__ */ _interopDefaultCompat(debounce);
...
function toSlateValue(value, { schemaTypes }, keyMap = {}) {
  return value && Array.isArray(value) ? value.map((block) => {
    ...
    if (block && block._type === schemaTypes.block.name) {
      ...
      return !hasMissingStyle && !hasMissingMarkDefs && !hasMissingChildren && !hasInlines && slate.Element.isElement(block) ? block : (hasMissingStyle && (rest.style = schemaTypes.styles[0].value), hasMissingMarkDefs && (rest.markDefs = EMPTY_MARKDEFS), keepObjectEquality({ _type, _key, ...rest, children }, keyMap));
    }
    ...
    if (operation.path.length === 1) {
      ...
      if (editor.isTextBlock(block)) {
        ...
        if (child) {
          ...
          return Object.keys(operation.newProperties).forEach((keyName) => {
  ...
  return function(editor) {
    ...
    return editor.normalizeNode = (nodeEntry) => {
      ...
      if (isSpan || isTextBlock) {
        isSpan && !Array.isArray(node.marks) && (debug$i("Adding .marks to span node"), slate.Transforms.setNodes(editor, { marks: [] }, { at: path }), editor.onChange());
      ...
      ) && normalizeMarkDefs(editor, types2);
    }, editor.apply = (op) => {
      var _a, _b;
      ...
      return slate.Range.isExpanded(editor.selection) && Array.from(slate.Editor.nodes(editor, { match: slate.Text.isText, at: editor.selection })).forEach(
        (n) => {
          const [node] = n;
          existingMarks = uniq__default.default([...existingMarks, ...node.marks || []]);
        }
      ), existingMarks.includes(mark);
    }, editor.pteToggleMark = (mark) => {
      editor.pteIsMarkActive(mark) ? (debug$i(`Remove mark '${mark}'`), slate.Editor.removeMark(editor, mark)) : (debug$i(`Add mark '${mark}'`), slate.Editor.addMark(editor, mark, !0));
    }, editor;
  };
    ...
    if (selection)
      for (const [node, path] of Array.from(
        ...
        if (editor.isTextBlock(parent)) {
          ...
          slate.Text.isText(node) && slate.Text.isText(nextNode) && isEqual__default.default(nextNode.marks, node.marks) && (debug$i("Merging spans"), slate.Transforms.mergeNodes(editor, { at: nextPath, voids: !0 }), editor.onChange());
        }
      }
  }
}
function normalizeMarkDefs(editor, types2) {
  const { selection } = editor;
  if (selection) {
    ...
    for (const [block, path] of blocks)
      if (editor.isTextBlock(block)) {
        const newMarkDefs = (block.markDefs || []).filter((def) => block.children.find((child) => slate.Text.isText(child) && Array.isArray(child.marks) && child.marks.includes(def._key)));
        isEqual__default.default(newMarkDefs, block.markDefs) || (debug$i("Removing markDef not in use"), slate.Transforms.setNodes(
          editor,
          {
  ...
  return function(editor) {
    return portableTextEditor.setEditable({
      ...
      insertChild: (type, value) => {
        ...
        return isSpanNode && focusNode._type !== types2.span.name && (debug$h("Inserting span child next to inline object child, moving selection + 1"), editor.move({ distance: 1, unit: "character" })), slate.Transforms.insertNodes(editor, child, {
          select: !0,
          at: editor.selection
      ...
      addAnnotation: (type, value) => {
        const { selection } = editor;
        if (selection) {
          const [block] = slate.Editor.node(editor, selection.focus, { depth: 1 });
          if (slate.Element.isElement(block) && block._type === types2.block.name) {
            ...
            if (editor.isTextBlock(block)) {
              slate.Transforms.setNodes(
                ...
                { at: selection.focus }
              ), editor.onChange(), slate.Range.isCollapsed(selection) && (editor.pteExpandToWord(), editor.onChange());
              const [textNode] = slate.Editor.node(editor, selection.focus, { depth: 2 });
              if (editor.selection) {
                slate.Editor.withoutNormalizing(editor, () => {
                  slate.Transforms.setNodes(editor, {}, { match: slate.Text.isText, split: !0 }), editor.selection && slate.Text.isText(textNode) && (slate.Transforms.setNodes(
                    editor,
                    {
                ...
                if (newSelection && typeof block._key == "string")
                  return slate.Editor.withoutNormalizing(editor, () => {
                    editor.selection && (slate.Transforms.insertNodes(
                      editor,
        ...
        if (selection) {
          ...
          if (range) {
            if (!(options != null && options.mode) || (options == null ? void 0 : options.mode) === "selected") {
              debug$h("Deleting content in selection"), slate.Transforms.delete(editor, {
                at: range,
                hanging: !0,
                voids: !0
              }), editor.onChange();
            ...
            (options == null ? void 0 : options.mode) === "blocks" && (debug$h("Deleting blocks touched by selection"), slate.Transforms.removeNodes(editor, {
              at: range,
              voids: !0,
              match: (node) => editor.isTextBlock(node) || !editor.isTextBlock(node) && slate.Element.isElement(node)
            })), (options == null ? void 0 : options.mode) === "children" && (debug$h("Deleting children touched by selection"), slate.Transforms.removeNodes(editor, {
              at: range,
              voids: !0,
              match: (node) => node._type === types2.span.name || // Text children
              !editor.isTextBlock(node) && slate.Element.isElement(node)
            })), editor.children.length === 0 && (editor.children = [editor.createPlaceholderBlock()]), editor.onChange();
          }
        }
      },
...
function insertPatch(editor, patch, schemaTypes) {
  ...
  if (patch.path.length === 1) {
    ...
    return debug$g(`Inserting blocks at path [${normalizedIdx2}]`), debugState(editor, "before"), slate.Transforms.insertNodes(editor, blocksToInsert, { at: [normalizedIdx2] }), debugState(editor, "after"), !0;
  }
  ...
  return debug$g(`Inserting children at path ${childInsertPath}`), debugState(editor, "before"), childrenToInsert && slate.Element.isElement(childrenToInsert[0]) && slate.Transforms.insertNodes(editor, childrenToInsert[0].children, { at: childInsertPath }), debugState(editor, "after"), !0;
}
...
function unsetPatch(editor, patch, previousPatch) {
  if (patch.path.length === 0) {
    ...
    }), slate.Transforms.insertNodes(editor, editor.createPlaceholderBlock()), previousSelection && slate.Transforms.select(editor, {
  ...
  if (patch.path.length === 1) {
    ...
    return debug$g(`Removing block at path [${blockIndex}]`), debugState(editor, "before"), slate.Transforms.removeNodes(editor, { at: [blockIndex] }), debugState(editor, "after"), !0;
  }
  return editor.isTextBlock(block) && patch.path[1] === "children" && patch.path.length === 3 ? !child || !childPath ? (debug$g("Child not found"), !1) : (debug$g(`Unsetting child at path ${JSON.stringify(childPath)}`), debugState(editor, "before"), debugVerbose$3 && debug$g(`Removing child at path ${JSON.stringify(childPath)}`), slate.Transforms.removeNodes(editor, { at: childPath }), debugState(editor, "after"), !0) : !1;
}
      ...
      if (undos.length > 0) {
        ...
        if (debug$f("Undoing", step), step.operations.length > 0) {
          ...
          try {
            ...
            debug$f("Could not perform undo step", err), remotePatches.splice(0, remotePatches.length), slate.Transforms.deselect(editor), editor.history = { undos: [], redos: [] }, SAVING.set(editor, !0), editor.onChange();
            return;
          }
      ...
      if (redos.length > 0) {
        ...
        if (debug$f("Redoing", step), step.operations.length > 0) {
          ...
          try {
            ...
            debug$f("Could not perform redo step", err), remotePatches.splice(0, remotePatches.length), slate.Transforms.deselect(editor), editor.history = { undos: [], redos: [] }, SAVING.set(editor, !0), editor.onChange();
            return;
          }
...
function createWithPlaceholderBlock({
  ...
  return function(editor) {
    return editor.createPlaceholderBlock = () => (debug$d("Creating placeholder block"), {
  ...
  return function(editor) {
    ...
    return editor.normalizeNode = (nodeEntry) => {
      ...
      for (const op of editor.operations)
        if (op.type === "split_node" && op.path.length === 1 && editor.isTextBlock(op.properties) && op.properties.style !== defaultStyle && op.path[0] === path[0] && !slate.Path.equals(path, op.path)) {
          ...
          if (slate.Text.isText(child) && child.text === "") {
            debug$c(`Normalizing split node to ${defaultStyle} style`, op), slate.Transforms.setNodes(editor, { style: defaultStyle }, { at: [op.path[0] + 1], voids: !1 });
            break;
          }
        }
      ...
      editor.selection && ([
        ...
        editor.isTextBlock(node) && node.style === blockStyle ? (debug$c(`Unsetting block style '${blockStyle}'`), slate.Transforms.setNodes(editor, { ...node, style: defaultStyle }, {
          at: path
        })) : (blockStyle ? debug$c(`Setting style '${blockStyle}'`) : debug$c("Setting default style", defaultStyle), slate.Transforms.setNodes(
          editor,
          {
            ...node,
...
function createWithPortableTextLists(types2) {
  return function(editor) {
    return editor.pteToggleListItem = (listItemStyle) => {
      ...
      editor.selection && [
        ...
        if (editor.isListBlock(node)) {
          ...
          debug$b(`Unsetting list '${listItemStyle}'`), slate.Transforms.setNodes(editor, newNode, { at: path });
        }
      });
      ...
      editor.selection && [
        ...
        debug$b(`Setting list '${listItemStyle}'`), slate.Transforms.setNodes(
          editor,
          {
            ...node,
            level: 1,
      ...
      return selectedBlocks.length === 0 ? !1 : (selectedBlocks.forEach(([node, path]) => {
        slate.Element.isElement(node) && (debug$b("Unset list"), slate.Transforms.setNodes(
          editor,
          {
            ...node,
            level: void 0,
      ...
      return selectedBlocks.length === 0 ? !1 : (selectedBlocks.forEach(([node, path]) => {
        if (editor.isListBlock(node)) {
          ...
          reverse ? (level--, debug$b("Decrementing list level", Math.min(MAX_LIST_LEVEL, Math.max(1, level)))) : (level++, debug$b("Incrementing list level", Math.min(MAX_LIST_LEVEL, Math.max(1, level)))), slate.Transforms.setNodes(
            editor,
    ...
    }, editor.pteHasListStyle = (listStyle) => editor.selection ? [
      ...slate.Editor.nodes(editor, {
        at: editor.selection,
...
function createWithUtils({ schemaTypes, keyGenerator }) {
  return function(editor) {
...
function createWithHotkeys(types$1, keyGenerator, portableTextEditor, hotkeysFromOptions) {
  const reservedHotkeys = ["enter", "tab", "shift", "delete", "end"], activeHotkeys = hotkeysFromOptions || DEFAULT_HOTKEYS, createEmptyBlock = () => toSlateValue(
    [
      {
  ...
  return function(editor) {
    return editor.pteWithHotKeys = (event) => {
      ...
      const isEnter = isHotkeyEsm.isHotkey("enter", event.nativeEvent), isTab = isHotkeyEsm.isHotkey("tab", event.nativeEvent), isShiftEnter = isHotkeyEsm.isHotkey("shift+enter", event.nativeEvent), isShiftTab = isHotkeyEsm.isHotkey("shift+tab", event.nativeEvent), isBackspace = isHotkeyEsm.isHotkey("backspace", event.nativeEvent), isDelete = isHotkeyEsm.isHotkey("delete", event.nativeEvent);
      if (isBackspace && editor.selection && editor.selection.focus.path[0] > 0 && slate.Range.isCollapsed(editor.selection)) {
        ...
        if (prevBlock && focusBlock && slate.Editor.isVoid(editor, prevBlock) && editor.selection.focus.offset === 0) {
          ...
          const isTextBlock = types.isPortableTextTextBlock(focusBlock), isEmptyFocusBlock = isTextBlock && focusBlock.children.length === 1 && ((_b = (_a = focusBlock.children) == null ? void 0 : _a[0]) == null ? void 0 : _b.text) === "";
      ...
      if (isEnter && !isShiftEnter && editor.selection) {
        ...
        if (editor.isTextBlock(focusBlock) && focusBlock.style && focusBlock.style !== types$1.styles[0].value) {
          ...
          if (slate.Editor.isEnd(editor, end, end.path)) {
            slate.Editor.insertNode(editor, createEmptyBlock()), event.preventDefault(), editor.onChange();
            return;
          }
        }
...
function validateValue(value, types$1, keyGenerator) {
  ...
  return value === void 0 ? { valid: !0, resolution: null, value } : !Array.isArray(value) || value.length === 0 ? {
    ...
    if (blk._type === types$1.block.name) {
      ...
      const orphanedMarks = uniq__default.default(
...
function _insertFragment(editor, fragment, schemaTypes) {
  ...
  isEqualToEmptyEditor(editor.children, schemaTypes) ? (slate.Transforms.splitNodes(editor, { at: [0, 0] }), editor.insertFragment(fragment), slate.Transforms.removeNodes(editor, { at: [0] })) : editor.insertFragment(fragment), editor.onChange();
}
const originalFnMap = /* @__PURE__ */ new WeakMap(), withPlugins = (editor, options) => {
  ...
  }), withPortableTextMarkModel = createWithPortableTextMarkModel(schemaTypes, change$), withPortableTextBlockStyle = createWithPortableTextBlockStyle(schemaTypes), withPlaceholderBlock = createWithPlaceholderBlock({
    keyGenerator,
    schemaTypes
  }), withUtils = createWithUtils({ keyGenerator, schemaTypes }), withPortableTextSelections = createWithPortableTextSelections(change$, schemaTypes);
  return e.destroy = () => {
...
function SlateContainer(props) {
  const { patches$, portableTextEditor, readOnly, maxBlocks, keyGenerator } = props, [slateEditor, subscribe] = react.useMemo(() => {
  ...
  const initialValue = react.useMemo(() => [slateEditor.createPlaceholderBlock()], [slateEditor]);
...
function useSyncValue(props) {
  ...
  }, [portableTextEditor]), updateValueDebounced = react.useCallback(
      ...
      if ((!value || value.length === 0) && (debug$4("Value is empty"), slate.Editor.withoutNormalizing(slateEditor, () => {
        withoutSaving(slateEditor, () => {
          withoutPatching(slateEditor, () => {
            ...
            }), slate.Transforms.insertNodes(slateEditor, slateEditor.createPlaceholderBlock(), { at: [0] }), hadSelection && slate.Transforms.select(slateEditor, [0, 0]);
          });
        });
...
function _replaceBlock(slateEditor, currentBlock, currentBlockIndex) {
  ...
  }), selectionFocusOnBlock && slate.Transforms.select(slateEditor, currentSelection);
}
    ...
    currentBlock.children.length < oldBlockChildrenLength && Array.from(Array(oldBlockChildrenLength - currentBlock.children.length)).forEach(
      ...
      if (isChildChanged)
        if (currentBlockChild._key === (oldBlockChild == null ? void 0 : oldBlockChild._key)) {
          ...
          oldBlockChild ? (debug$4("Replacing child", currentBlockChild), slate.Transforms.removeNodes(slateEditor, {
...
__publicField(PortableTextEditor, "activeAnnotations", (editor) => editor && editor.editable ? editor.editable.activeAnnotations() : []), __publicField(PortableTextEditor, "addAnnotation", (editor, type, value) => {
  var _a;
...
const debug = debugWithName("component:Editable"), PLACEHOLDER_STYLE = {
  ...
  left: 0,
  right: 0
}, EMPTY_DECORATORS = [], PortableTextEditable = react.forwardRef(function(props, forwardedRef) {
  const {
    hotkeys,
    onBlur,
    onFocus,
    ...
    spellCheck,
    ...restProps
  } = props, portableTextEditor = usePortableTextEditor(), readOnly = usePortableTextEditorReadOnlyStatus(), keyGenerator = usePortableTextEditorKeyGenerator(), ref = useForwardedRef(forwardedRef), [editableElement, setEditableElement] = react.useState(null), [hasInvalidValue, setHasInvalidValue] = react.useState(!1), [rangeDecorationState, setRangeDecorationsState] = react.useState(EMPTY_DECORATORS), { change$, schemaTypes } = portableTextEditor, slateEditor = slateReact.useSlate(), blockTypeName = schemaTypes.block.name, withInsertData = react.useMemo(
  ...
  }, [change$, restoreSelectionFromProps, syncRangeDecorations]), react.useEffect(() => {
    propsSelection && !hasInvalidValue && restoreSelectionFromProps();
  }, [hasInvalidValue, propsSelection, restoreSelectionFromProps]);
  const originalApply = react.useMemo(() => slateEditor.apply, [slateEditor]);
  react.useEffect(() => {
  ...
  const handleKeyDown = react.useCallback(
    ...
    ([, path]) => {
      ...
      const result = rangeDecorationState.filter((item) => slate.Range.includes(item, path));
      return result.length > 0 ? result : EMPTY_DECORATORS;
    },
