...
function createWithPortableTextMarkModel(types, change$) {
  return function(editor) {
    ...
    return editor.normalizeNode = (nodeEntry) => {
      ...
      return Range.isExpanded(editor.selection) && Array.from(Editor.nodes(editor, { match: Text.isText, at: editor.selection })).forEach(
        (n) => {
          const [node] = n;
...
function unsetPatch(editor, patch, previousPatch) {
  if (patch.path.length === 0) {
    ...
    }), Transforms.insertNodes(editor, editor.createPlaceholderBlock()), previousSelection && Transforms.select(editor, {
...
function createWithHotkeys(types, keyGenerator, portableTextEditor, hotkeysFromOptions) {
  ...
  return function(editor) {
    return editor.pteWithHotKeys = (event) => {
      ...
      const isEnter = isHotkey("enter", event.nativeEvent), isTab = isHotkey("tab", event.nativeEvent), isShiftEnter = isHotkey("shift+enter", event.nativeEvent), isShiftTab = isHotkey("shift+tab", event.nativeEvent), isBackspace = isHotkey("backspace", event.nativeEvent), isDelete = isHotkey("delete", event.nativeEvent);
      if (isBackspace && editor.selection && editor.selection.focus.path[0] > 0 && Range.isCollapsed(editor.selection)) {
        ...
        if (prevBlock && focusBlock && Editor.isVoid(editor, prevBlock) && editor.selection.focus.offset === 0) {
          ...
          const isTextBlock = isPortableTextTextBlock(focusBlock), isEmptyFocusBlock = isTextBlock && focusBlock.children.length === 1 && ((_b = (_a = focusBlock.children) == null ? void 0 : _a[0]) == null ? void 0 : _b.text) === "";
      ...
      if (isEnter && !isShiftEnter && editor.selection) {
        ...
        if (editor.isTextBlock(focusBlock) && focusBlock.style && focusBlock.style !== types.styles[0].value) {
          ...
          if (Editor.isEnd(editor, end, end.path)) {
            Editor.insertNode(editor, createEmptyBlock()), event.preventDefault(), editor.onChange();
            return;
          }
        }
...
function SlateContainer(props) {
  ...
  const initialValue = useMemo(() => [slateEditor.createPlaceholderBlock()], [slateEditor]);
  return useEffect(() => () => {
      ...
      if ((!value || value.length === 0) && (debug$4("Value is empty"), Editor.withoutNormalizing(slateEditor, () => {
        withoutSaving(slateEditor, () => {
          withoutPatching(slateEditor, () => {
            ...
            }), Transforms.insertNodes(slateEditor, slateEditor.createPlaceholderBlock(), { at: [0] }), hadSelection && Transforms.select(slateEditor, [0, 0]);
          });
        });
    ...
    currentBlock.children.length < oldBlockChildrenLength && Array.from(Array(oldBlockChildrenLength - currentBlock.children.length)).forEach(
      ...
      if (isChildChanged)
        if (currentBlockChild._key === (oldBlockChild == null ? void 0 : oldBlockChild._key)) {
          ...
          oldBlockChild ? (debug$4("Replacing child", currentBlockChild), Transforms.removeNodes(slateEditor, {
...
const debug = debugWithName("component:Editable"), PLACEHOLDER_STYLE = {
  ...
  left: 0,
  right: 0
}, EMPTY_DECORATORS = [], PortableTextEditable = forwardRef(function(props, forwardedRef) {
  const {
    hotkeys,
    onBlur,
    onFocus,
    ...
    spellCheck,
    ...restProps
  } = props, portableTextEditor = usePortableTextEditor(), readOnly = usePortableTextEditorReadOnlyStatus(), keyGenerator = usePortableTextEditorKeyGenerator(), ref = useForwardedRef(forwardedRef), [editableElement, setEditableElement] = useState(null), [hasInvalidValue, setHasInvalidValue] = useState(!1), [rangeDecorationState, setRangeDecorationsState] = useState(EMPTY_DECORATORS), { change$, schemaTypes } = portableTextEditor, slateEditor = useSlate(), blockTypeName = schemaTypes.block.name, withInsertData = useMemo(
  ...
  }, [change$, restoreSelectionFromProps, syncRangeDecorations]), useEffect(() => {
  ...
  const handleKeyDown = useCallback(
    ...
    ([, path]) => {
      ...
      const result = rangeDecorationState.filter((item) => Range.includes(item, path));
